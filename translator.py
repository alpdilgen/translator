...
# Function to log extended content (like prompts) to file only
def log_extended_content(content, content_type="prompt", batch_num=None):
    """Log extended content like prompts to the log file only (not UI)"""
    if batch_num is not None:
        logger.info(f"======== BEGIN {content_type.upper()} (BATCH {batch_num}) ========")
    else:
        logger.info(f"======== BEGIN {content_type.upper()} ========")
    
    # Log the content line by line to avoid very long lines
    for line in content.split('\n'):
        logger.info(line)
    
    if batch_num is not None:
        logger.info(f"======== END {content_type.upper()} (BATCH {batch_num}) ========")
    else:
        logger.info(f"======== END {content_type.upper()} ========")
...
def create_ai_prompt(prompt_template, source_lang, target_lang, document_name, batch, tm_matches, term_matches):
    ...
    try:
        ...
        # Log the complete prompt to the log file
        current_batch = None
        if hasattr(st.session_state, 'current_batch'):
            current_batch = st.session_state.current_batch
        
        log_extended_content(prompt, "prompt", current_batch)
...
def get_ai_translation(api_provider, api_key, model, prompt, source_lang, target_lang, temperature=0.3):
    ...
    # Log that we're about to send the prompt
    log_message(f"Sending prompt to {api_provider} API")
    ...
    try:
        if api_provider == 'anthropic':
            ...
            log_extended_content(system_prompt, "system_prompt")
            ...
            log_extended_content(response_content, "ai_response", st.session_state.current_batch)
            ...
            try:
                ...
                if prompt_file:
                    try:
                        ...
                        log_extended_content(prompt_template, "prompt_template")
                ...
                if custom_prompt_text:
                    ...
                    # Log the updated prompt template with custom text
                    if custom_prompt_text:
                        log_message("Custom prompt text added:")
                        log_extended_content(custom_prompt_text, "custom_prompt_text")
